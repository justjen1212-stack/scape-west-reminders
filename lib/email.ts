              ...
              <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
                ...
                <tr>
                  <td style="padding:40px;">
                    ...
                    <p style="color:#5c3d1e;font-size:16px;line-height:1.6;margin:0 0 16px;">${intro}</p>
                    <p style="color:#5c3d1e;font-size:16px;line-height:1.6;margin:0 0 24px;">${t.main_paragraph}</p>
                    ...
                    <p style="color:#5c3d1e;font-size:16px;line-height:1.6;margin:0 0 16px;">${t.recommendation}</p>
                    <p style="color:#5c3d1e;font-size:16px;line-height:1.6;margin:0 0 32px;">${t.closing}</p>
                ...
                <tr>
                  <td style="background-color:#f0e8d8;padding:24px 40px;text-align:center;">
                    <p style="color:#8b7355;font-size:13px;margin:0;">
                      ...
                      Questions? Email us at <a href="mailto:info@scape-west.co.uk" style="color:#8b5e3c;">info@scape-west.co.uk</a>
                    </p>
                  </td>
                </tr>
              </table>
...
export async function sendWaxReminderEmail({
  ...
  const html = buildCustomerEmailHtml({ firstName, intro, t });

  return getTransporter().sendMail({
    from: `"Scape West" <${process.env.IONOS_EMAIL}>`,
    to,
    subject: t.subject,
    html,
  });
...
export async function sendOwnerApprovalEmail({
  ...
  // Build a preview using the first customer as the example
  const first = customers[0];
  const previewFirstName = first.customerName.split(' ')[0];
  const previewIntro = t.intro.replace('[product]', first.productNames);
  const emailPreviewHtml = buildCustomerEmailHtml({
    ...
    <html>
      ...
      <body style="margin:0;padding:0;background-color:#f9f6f1;font-family:Georgia,serif;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9f6f1;padding:40px 0;">
          <tr>
            <td align="center">
              <table width="660" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
                ...
                <tr>
                  <td style="padding:32px 40px;">
                    ...
                    <!-- Email Preview -->
                    <p style="color:#2c1a0e;font-size:14px;font-weight:bold;text-transform:uppercase;letter-spacing:1px;margin:0 0 12px;">Email Preview</p>
                    <p style="color:#8b7355;font-size:13px;margin:0 0 16px;">This is exactly what your customers will receive:</p>
                    <div style="border:2px solid #e8ddd0;border-radius:8px;overflow:hidden;margin-bottom:32px;">
                      ${emailPreviewHtml}
                    </div>
